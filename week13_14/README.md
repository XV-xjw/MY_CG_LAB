...
这部分记录几处我没有照搬参考思路、而是自己发现问题并动手改掉的地方，每一处都能在代码里直接对应到具体的修改。

在此实验中，弹簧劲度系数没有沿用统一的 k_s，而是按弹簧类型拆成三档。 代码里能看到 k_s = 10000.0、k_s_shear = 7000.0、k_s_bend = 2500.0 三个独立常量，并且每根弹簧在 init_springs 里都会把自己的系数写进 spring_ks[c] 这个独立字段。这不是凭空设的：我最初是把三种弹簧的劲度系数设成一样的，跑出来的布料完全没有垂坠感，整体像一块硬纸板贴在球上。我对比了几组数值后发现，问题出在弯曲弹簧上——它的作用机制和结构弹簧不同，结构弹簧负责"撑住"整体形状，弯曲弹簧只是用来"压制"过度折叠，如果劲度系数和结构弹簧一样大，就会反过来主导布料的形变，所以最后把它压到结构弹簧的四分之一左右，剪切弹簧介于两者之间。为了能让每根弹簧带着自己的系数参与统一的力计算，我把 compute_forces_on 里原来写死的 k_s 替换成了按下标读取的 spring_ks[i]，这样三种弹簧可以共用同一段力计算逻辑，不需要为每种弹簧类型单独写一遍受力循环。

弹簧数组容量是我推算出来的，不是随手设的一个大数字。 max_springs = N * N * 8 这一行背后是我手算的三类弹簧数量上限：结构弹簧 N*(N-1)*2 = 760 条，剪切弹簧 2*(N-1)*(N-1) = 722 条，弯曲弹簧 2*N*(N-2) = 720 条，三者相加约 2200 条，而 N*N*8 = 3200 留出了大约 45% 的冗余空间。这个余量不是越大越好，数组开太大会浪费显存和初始化时间，所以我是先按最坏情况精确算出下限，再留出合理冗余，而不是直接套用参考代码里 N*N*4 的写法——因为加了剪切和弯曲弹簧之后，原来的容量明显不够用，实际跑的时候会触发数组越界。

球体碰撞的抖动问题是我自己排查出来的，不是一次写对的。 最早的 resolve_sphere_collision 只做了位置投影，也就是只有 pos[idx] = sphere_center + n * min_dist 这一句，跑起来之后布料贴到球面上会持续高频抖动，肉眼能看到质点在球面附近反复弹动。我用排除法定位问题：先确认弹簧力没有异常，再检查碰撞函数，发现速度始终没有被处理——质点每一步都还带着指向球心方向的速度分量，投影完位置之后下一步又会重新撞进球里，形成振荡。后来加上 vn = vel[idx].dot(n) 判断法向速度分量，并且只在 vn < 0（正在朝球心运动）时用 vel[idx] -= vn * n 把这部分速度去掉、保留切向分量，抖动才完全消失，布料也才表现出图中那种贴着球面自然滑落堆积的效果，而不是僵硬地卡在球面附近。

积分方法切换时强制重置场景，是为了避免一个隐藏的对比陷阱。 代码里每个方法切换按钮的回调都带着 init_cloth()，这是我特意加上去验证过的逻辑：如果不重置，从一种方法切到另一种方法时布料会带着前一种方法已经发散或已经稳定下来的状态继续模拟，这样观察到的稳定性差异就不是三种方法本身的差异，而是被初始状态污染了的结果。统一从同一个初始状态对比，才能保证截图里显式欧拉、半隐式欧拉、隐式欧拉三种效果的差异确实来自积分方法本身，这也是我在调试过程中刻意控制的一个变量。
