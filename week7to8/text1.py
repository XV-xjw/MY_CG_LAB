...
def render():
    for i, j in pixels:
        ...
        # 1. 渲染红球 (放在左边)
        t_sph, n_sph = intersect_sphere(ro, rd, ti.Vector([-2.0, -0.5, 0.0]), 1.0)  # 向左移动，减小半径
        if 0 < t_sph < min_t:
            min_t = t_sph
            hit_normal = n_sph
        ...
        # 2. 渲染紫色圆锥 (放在右边)
        # 顶点在 y=1.2，底面在 y=-1.4
        t_cone, n_cone = intersect_cone(ro, rd, ti.Vector([2.0, 1.2, 0.0]), -1.4, 1.0)  # 向右移动，减小半径
        if 0 < t_cone < min_t:
            min_t = t_cone
            hit_normal = n_cone
        ...
        # 3. 渲染蓝色圆锥 (更高，放在中间)
        # 顶点在 y=2.0，底面在 y=-1.0（更高）
        t_cone_blue, n_cone_blue = intersect_cone(ro, rd, ti.Vector([0.0, 2.0, 0.0]), -1.0, 0.8)  # 保持在中间，减小半径
        if 0 < t_cone_blue < min_t:
            min_t = t_cone_blue
            hit_normal = n_cone_blue
            hit_color = ti.Vector([0.2, 0.4, 0.8])  # 蓝色

        # 背景色
        color = ti.Vector([0.05, 0.2, 0.1])  # 暗绿色
