...
spp = ti.field(ti.i32, shape=()) # 新增：控制抗锯齿的采样次数
...
def intersect_sphere(ro, rd, center, radius):
    ...
    if delta > 0:
        ...
        t2 = (-b + ti.sqrt(delta)) / 2.0 # 需要记录更远的交点，光线穿出玻璃时会用到
        if t1 > 0:
            t = t1
            p = ro + rd * t
...
def scene_intersect(ro, rd):
    ...
    if 0 < t < min_t:
        min_t = t
        hit_n = n
        hit_c = ti.Vector([1.0, 0.8, 0.8]) # 给玻璃带一点微弱的粉红色滤镜
        hit_mat = MAT_GLASS
...
def render():
    ...
    for i, j in pixels:
        ...
        # 💡【核心关键点 2】：抗锯齿的多重采样 (MSAA)
        for sample in range(spp[None]):
            # 在一个像素格内引入 ti.random() 进行随机偏移，打破锯齿
            ...
            for bounce in range(max_bounces[None]):
                ...
                # 💡【核心关键点 3】：玻璃材质与内部光线追踪
                    ...
                    dot_I_N = rd.dot(N)
                    N_eff = N
                    eta_ratio = 1.0 / 1.5 # 空气折射率(1.0) / 玻璃折射率(1.5)
                    
                    # 判断光线是从外部射入，还是在内部准备射出
                    if dot_I_N > 0: 
                        N_eff = -N # 在内部，法线翻转
                        eta_ratio = 1.5 / 1.0 # 玻璃射向空气
                    ...
                    if is_tir == 1:
                        ...
                        # 正常折射穿透表面
                        ro = p - N_eff * 1e-4 # 注意！向法线的【反方向】偏移，让光线穿过去
                        rd = normalize(ref_dir)
                    
                    throughput *= 0.95 * obj_color # 玻璃透光率很高
            ...
            pixel_color += final_color # 累加当前像素多次采样的颜色
